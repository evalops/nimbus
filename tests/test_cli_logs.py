from __future__ import annotations

import json

import pytest

from nimbus.cli import logs


@pytest.mark.asyncio
async def test_cli_logs_prints_entries(monkeypatch, capsys, logs_cli_runner):
    entries = [
        {"timestamp": "t1", "job_id": 1, "level": "info", "message": "hello"},
        {"timestamp": "t2", "job_id": 2, "level": "error", "message": "fail"},
    ]

    async def fake_fetch(url: str, params: dict[str, object]):  # noqa: ANN001
        assert params["limit"] == 50
        return entries

    logs_cli_runner(limit=50, json=False)
    monkeypatch.setattr(logs, "fetch_logs", fake_fetch)

    await logs.run()
    output = capsys.readouterr().out
    assert "job=1" in output
    assert "hello" in output
    assert "job=2" in output


@pytest.mark.asyncio
async def test_cli_logs_json(monkeypatch, capsys, logs_cli_runner):
    async def fake_fetch(url: str, params: dict[str, object]):  # noqa: ANN001
        return [
            {"timestamp": "t3", "job_id": 3, "level": "info", "message": "done"}
        ]

    logs_cli_runner(job_id=5, contains="abc", limit=10, json=True)
    monkeypatch.setattr(logs, "fetch_logs", fake_fetch)

    await logs.run()
    output = capsys.readouterr().out
    payload = json.loads(output)
    assert payload[0]["job_id"] == 3


@pytest.mark.asyncio
async def test_cli_logs_no_entries(monkeypatch, capsys, logs_cli_runner):
    async def fake_fetch(url: str, params: dict[str, object]):  # noqa: ANN001
        return []

    logs_cli_runner(limit=5, json=False)
    monkeypatch.setattr(logs, "fetch_logs", fake_fetch)

    await logs.run()
    output = capsys.readouterr().out.strip()
    assert output == "No log entries found"
